...
@section('content')
                                    ...
                                    @foreach ($dataTypeContent as $data)
                                        <tr>
                                            ...
                                            <th>
                                                <a href="{{ url('/generate-pdf/' . $data->id . '') }}"
                                                    ...
                                                    <i class="glyphicon glyphicon-file"></i> <span
                                                        class="export-to-pdf">Export to pdf</span>
                                                </a>
                                            </th>
                                            <td>

                                                <?php
                                                if ($data->order_state == 'ordered') {
                                                    $color = '#ffdb0c';
                                                } elseif ($data->order_state == 'processing') {
                                                    $color = '#0cfffd';
                                                ...
                                                ?>
                                                <p id="order_status{{ $data->id }}"
                                                    style="background-color: {{ $color }}; border-radius:5px;padding:5px;text-align:center;">
                                                    {{ $data->order_state }}
                                                </p>
                                            </td>
                                            ...
                                            <td>
                                                ...
                                                <form action="">
                                                    @csrf
                                                    <select name="" id="ajaxStatus" class="form-control ajaxStatus">

                                                        <option value="processing_{{ $data->id }}"
                                                            {{ $data->order_state == 'processing' ? 'selected' : '' }}>
                                                            processing</option>
                                                        <option value="cancelled_{{ $data->id }}"
                                                            {{ $data->order_state == 'cancelled' ? 'selected' : '' }}>
                                                            cancelled</option>
                                                        <option value="completed_{{ $data->id }}"
                                                            {{ $data->order_state == 'completed' ? 'selected' : '' }}>
                                                            completed</option>
                                                    </select>
                                                </form>
                                                {{-- <form action="{{ url('/') . '/processing-order/' . $data->id }}"
                                                    method="POST">
                                                    @csrf
                                                    <input type="submit" class="form-control" value="Processing">
                                                </form>

                                                <form action="{{ url('/') . '/cancel-order/' . $data->id }}"
                                                    method="POST">
                                                    @csrf
                                                    <input type="submit" class="form-control" value="cancelled">
                                                </form>
                                                <form action="{{ url('/') . '/complete-order/' . $data->id }}"
                                                    method="POST">
                                                    @csrf
                                                    <input type="submit" class="form-control" value="completed">
                                                </form> --}}
                                            </td>
    ...
    <div id="dialog-modal" style=" text-align: center;  background: #fafafa; border: 4px solid #755245;
                                                    border-radius: 20px;
                                                    padding: 20px;
                                                ">
        ...
        <form action="">
            ...
            <div class="form-group" style="padding-right: 20px; padding-left: 20px;">
                <label for="exampleInputEmail1">اكتب ملاحظاتك</label>
                <textarea type="text" name="manager_notes" id="manager_notes" class="form-control" aria-describedby="emailHelp"
                    placeholder="..."> </textarea>
                <small id="emailHelp" class="form-text text-muted"> ملاحظاتك ستظهر للعميل </small>
            </div>
...
@section('javascript')
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.ajaxStatus').on('change', function(e) {
                var strValue = $(this).val().split("_")
                var orderStatus = strValue[0]
                var orderId = strValue[1]
              
                if (orderStatus == 'cancelled') {
                    e.preventDefault();
                    jQuery.ajax({
                        url: '/spices-sys/cancel-order/' + orderId,
                        method: 'post',
                    ...
                    $('#order_status' + orderId).html('cancelled')
                    $('#order_status' + orderId).css('background-color', '#ff560c');

                } else if (orderStatus == 'processing') {
                    e.preventDefault();
                    jQuery.ajax({
                        url: '/spices-sys/processing-order/' + orderId,
                        method: 'post',
                        success: function(result) {
                    ...
                    $('#order_status' + orderId).html('processing')
                    $('#order_status' + orderId).css('background-color', '#0cfffd');
                } else if (orderStatus == 'completed') {
                    e.preventDefault();
                    jQuery.ajax({
                        url: '/spices-sys/complete-order/' + orderId,
                        method: 'post',
                        success: function(result) {
                            jQuery('.alert').show();
                            jQuery('.alert').html(result.success);
                    ...
                    $('#order_status' + orderId).html('completed')
                    $('#order_status' + orderId).css('background-color',  '#0cff12');
                }

                $('#dialog-modal')
                    .data('param_order_id', orderId)
                    .dialog('open');

                $('#param_1').val($("#dialog-modal").data('param_order_id'));
                // alert($("#dialog-modal").data('param_order_id'))
            });
            ...
            jQuery('#addNotesAjax').click(function(e) {
                ...
                e.preventDefault();
                jQuery.ajax({
                    url: '/spices-sys/update-order-add-notes/' + $("#dialog-modal").data(
                        'param_order_id'),
                    method: 'post',
                    data: {
                        manager_notes: jQuery('#manager_notes').val(),
                    ...
                    success: function(result) {
                        ...
                        $('#dialog-modal')
                            ...
                            alert('تم إرسال الملاحظة للطلبية رقم : ' +  $("#dialog-modal").data(
                        'param_order_id'))
                    }
                });
