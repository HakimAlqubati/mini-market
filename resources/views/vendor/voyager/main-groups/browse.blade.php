...
{{-- @include('vendor.voyager.main-groups.modal-create') --}}
...
@section('page_header')
    <div class="container-fluid">
        ...
        @can('add', app($dataType->model_name))
            ...
            <button type="button" class="btn btn-success btn-add-new" data-toggle="modal" data-target="#add-modal">
                {{ __('add_new_button') }}
                                ...
                                <tbody>
                                    @foreach ($dataTypeContent as $data)
                                        <tr>
                                                ...
                                                <td>
                                                    ...
                                                    @elseif($row->type == 'relationship')
                                                        @include(
                                                            ...
                                                            ['view' => 'browse', 'options' => $row->details]
                                                        )
                                            ...
                                            <td>
                                                <input type="checkbox" {{ $data->active == 1 ? 'checked' : '' }}
                                                    onclick="return false;">
                                            </td>
                                            <td class="no-sort no-click bread-actions">

                                                {{-- <a href="javascript:;" class="btn btn-success btn-add-new"
                                                    value="{{ $data->getKey() }}" data-toggle="modal"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel"> {{ __('main_group_modal_title') }}</h5>
                ...
                <div class="modal-body">
                    ...
                    <form action="{{ url('/') }}/store-main-group"
                    
                        method="POST" enctype="multipart/form-data">
                        {{ csrf_field() }}
                        @csrf
                        <div class="col-md-4">
                            <label for="validationCustom01"> {{ __('main_group_name_label') }}</label>
                            <input type="text" name="name" class="form-control" id="validationCustom01"
                                placeholder=" {{ __('main_group_name_hint') }}" required>

                        </div>
                        <div class="col-md-4">
                            <label for="validationCustom01"> {{ __('english_name_label') }}</label>
                            <input type="text" name="english_name" class="form-control" id="validationCustom01"
                                placeholder=" {{ __('english_name_label') }}" required>

                        </div>


                        <div class="col-md-4">
                            <label for="validationCustom01"> {{ __('main_group_description_label') }}</label>
                            <input type="text" name="desc" class="form-control" id="validationCustom01"
                                placeholder=" {{ __('main_group_name_hint') }}" required>
                        ...
                        <div class="col-md-12" style="text-align: center">
                            ...
                            <input class="btn btn-info" type="submit" value=" {{ __('save') }}" />
                        </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form action="http://localhost/spices-sys/store-main-group" method="POST" enctype="multipart/form-data">
                        {{ csrf_field() }}
                        @csrf
                        <div class="col-md-4">
                            <label for="validationCustom01"> {{ __('main_group_name_label') }}</label>
                            <input type="text" name="name" class="form-control" id="validationCustom01"
                                placeholder="group name here" required>
    ...
    true,
        ...
        @if ($usesSoftDeletes)
            ...
            $('#dataTable').before('<a id="redir"
                href="{{ route('voyager.' . $dataType->slug . '.index', array_merge($params, ['showSoftDeleted' => 1]), true) }}"></a>');
            }else{
            $('#dataTable').before('<a id="redir"
