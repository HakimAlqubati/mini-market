...
class OrderController extends  VoyagerBaseController
{
    ...
    {
        $request->user()->settings =  '{"locale":"en"}';
        $request->user()->save();
        return redirect()->back();
    }
    public function setArabic(Request $request)
    {
        $request->user()->settings =  '{"locale":"ar"}';
        $request->user()->save();
        return redirect()->back();
    }
    ...
    {
        if ($request->all() != null && $request->customer != null) {

            $strSelect = "select orders.id as order_id , orders.order_state , orders.created_by as customer_id , 
        users.name as customer_name ,  order_items.product_id , products.name as product_name , products.code as product_code , order_items.unit_id , units.name as unit_name
       , sum(order_items.qty) as qty
       , order_items.price, orders.created_at  
        from orders
        LEFT JOIN order_items ON (order_items.order_id = orders.id)
        LEFT JOIN products ON (products.id = order_items.product_id)
        LEFT JOIN units ON (units.id = order_items.unit_id)
        LEFT JOIN users ON (users.id = orders.created_by)";
            ...
            $strSelect .= " where orders.created_at between '$from_date' AND '$to_date' and orders.created_by = '$request->customer' ";

            if ($request->order_state) {
                $state_name = $request->order_state;
                $strSelect .= " and orders.order_state = '$request->order_state'";
            } else {
                $state_name = 'All';
            }

            $strSelect .= 'group by order_items.product_id , order_items.unit_id , orders.order_state';

            $final = DB::select($strSelect);
        } else if ($request->all() != null && $request->customer == null) {

            // dd("DDDDDDDD");
            return redirect()->back()->with([
                'message'    =>   "You have to select customer",
                'alert-type' => 'error',
            ]);
        } else {
            $final = [];
        ...
        if (User::where('id', $request->customer)->first()) {

            $customerName = User::where('id', $request->customer)->first()->name;
        } else {
            $customerName = null;
        }

        return view('voyager::orders.details-report-customer', compact(
            'final',
            'from_date',
            'to_date',
            'state_name',
    ...
    public function searchReportCustomer(Request $request)
    {

        $orders = [];
        if ($request->all() != null && $request->customer != null) {
            $orders = Order::where('created_by', $request->customer)->get();
        }
        ...
        return view(
            'voyager::orders.search-report-customer',
            compact(
                'customersData',
                'orders'
            )
    ...
    public function report(Request $request)
    {
       ...
       , order_items.price, orders.created_at  
        ...
        $strSelect .= " where products.active = 1 ";

        if ($request->code) {
            $strSelect .= " and products.code = '$request->code' ";
        }
        ...
        return view('voyager::orders.report', compact(
            'final',
            'from_date',
            'to_date',
            'state_name'
    ...
    public function reportBasedOnProduct(Request $request)
    {

        $strSelect = "select orders.id as order_id , orders.order_state , orders.created_by as customer_id , 
        users.name as customer_name ,  order_items.product_id , products.name as product_name ,products.code as product_code ,
         order_items.unit_id , units.name as unit_name  ,  order_items.qty 
       , order_items.price, orders.created_at  
        from orders
        LEFT JOIN order_items ON (order_items.order_id = orders.id)
        LEFT JOIN products ON (products.id = order_items.product_id)
        ...
        return view('voyager::orders.report-product', compact(
            'final',
            'state_name'
        ));
    }
    ...
    {
        $data = OrderItems::where('order_id', $orderId)->get();
    ...
    public function show(Request $request, $id)
    {
        ...
        $notification = DB::table('notifications')->where('notifiable_id', $request->user()->id)->where('read_at', null)->get();
        foreach ($notification as $key => $value) {
            if ($id == json_decode($value->data)->order_id) {
        ...
        foreach ($order as   $val) {
            $finalResultOrder = [
                ...
                'user_name' =>  User::where('id', $val->created_by)->get()[0]->name,
            ];
        }



        $orderDetails = OrderItems::where('order_id', $order[0]->id)->get();
        foreach ($orderDetails as $key => $value) {
            $finalResultOrderItems[] = [
                'product_id' =>  $value->product_id,
                'product_name' =>  Product::where('id', $value->product_id)->first()->name,
                'product_code' =>  Product::where('id', $value->product_id)->first()->code,
                'unit_id' =>  $value->unit_id,
                'unit_name' =>  Unit::where('id', $value->unit_id)->first()->name,
                'price' =>  $value->price,
                'qty' =>  $value->qty,
            ];
        }
        ...
        return Voyager::view('voyager::orders.show', compact(
            'dataType',
            'dataTypeContent',
            'isModelTranslatable',
            'orderStates',
    ...
    public function generatePDF(Request $request, $id)
    {
        ...
        $data = [
            ...
            'orderItemsData' => OrderItems::where('order_id', $id)->get()
        ];
        ...
        // return  view('myPDF', compact('data', 'orderData', 'orderItemsData'));
