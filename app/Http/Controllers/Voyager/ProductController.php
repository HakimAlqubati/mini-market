...
class ProductController extends VoyagerBaseController
{
    ...
    public function createColors(Request $request, $id)
    {


        $unitPrices = UnitPrice::where('product_id', $id)->get();

        $productName = Product::find($id)->name;
        return view('voyager::products.create-colors-to-units', compact('unitPrices', 'productName'));
    }
    public function addColors(Request $request, $id)
    {
        $unitPrice = UnitPrice::find($id);
        $unitPrice->colors =   implode(',', $request->colors);
        $update =  $unitPrice->save();
        return $update;
    }
    ...
    public function anActiveProduct(Request $request, $id)
    {
        $product = Product::find($id);
        $product->active =  0;
        $product->save();
    ...
    public function edit(Request $request, $id)
    {
        ...
        $unit_prices = UnitPrice::where('product_id', $id)->get();
    ...
    public function getUnitPricesToDelete($product_id)
    {

        $data =  UnitPrice::where('product_id', $product_id)->get();
        if (count($data) > 0) {
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->file('image') && $request->file('image')->getClientOriginalExtension() == 'svg') {
            return redirect()->back()->with([
                'message'    => 'SVG is not allowed',
                'alert-type' => 'error',
            ]);
        }
        ...
        if ($request->images) {
            for ($i = 0; $i < count($request->images); $i++) {
                ...
                Storage::disk('local')->put('public/products' . '/' . $fileName, $img, 'public');
        ...
        if (isset($arrImages)) {
            // dd(implode(',', $arrImages));
            $product->image = implode(',', $arrImages);
        }
        ...
        $product->group_id = $request->sub_group;
        $product->active = $request->active != null ? 1 : 0;
        ...
        if (array_key_exists('unit_price_id', $request->all()) == true) {
            ...
            $all_exist_unit_prices = $this->getUnitPricesToDelete($id);



            $array_diff = array_diff($all_exist_unit_prices, $request->all()['unit_price_id']);
            ...
            if (count($array_diff) > 0) {
                ...
                foreach ($array_diff as   $unit_price_id) {
                    UnitPrice::where('id', $unit_price_id)->delete();
                }
            }
            ...
            foreach ($request->unit as $key => $value) {
                $unitPriceId = $request->unit_price_id[$key];

                $unit = $request->unit[$key];
                $price = $request->price[$key];

                $unit_price = UnitPrice::find($unitPriceId);
                if ($unitPriceId != null) {
                    $unit_price->unit_id =  $unit;
        ...
        // dd(array_key_exists('unit_price_id', $request->all()));
        if (array_key_exists('unit_price_id', $request->all()) == false) {

            $all_exist_unit_prices = $this->getUnitPricesToDelete($id);
            if ($all_exist_unit_prices != null && count($all_exist_unit_prices) > 0) {

                foreach ($all_exist_unit_prices as   $unit_price_id) {
                    UnitPrice::where('id', $unit_price_id)->delete();
                }
            }

            if ($request->unit != null && count($request->unit)) {
    ...
    public function create(Request $request)
    {
        ...
        $mainGroups = MainGroup::where('active', 1)->get();

        $subGroups = SubGroup::where('active', 1)->get();
    ...
    public function store(Request $request)
    {
        ...
        $product = new Product(
            [
                ...
                'code' => $request->code,
                'group_id' => $request->sub_group,
                'active' => $request->active,
                'image' =>  implode(',', $arrImages)
            ]
        );
        ...
        foreach ($request->unit as $key => $value) {
            ...
            // $colors[] = $request->colors[$key];
            $obj = new stdClass();
            $obj->unit_id = $unit;
            $obj->price = $price;
            $obj->product_id  = $product->id;

            // $obj->colors  = implode(',', $request->all()['colors']);
        ...
        $details = json_decode(json_encode($details), true);
        // dd($details[0]['unit_id']);

        // $isDupplicate = false;
        // $i = 0;
        // foreach ($details as $key => $value) {
        //     if ($i > 0) {
        //         dd($details[$i]['unit_id'], $value['unit_id']);
        //     }
        //     if (
        //         $i > 0 &&
        //         (($details[$i + 1]['unit_id'] == $value['unit_id']))
        //         // ||
        //         // ($details[$i - 1]['unit_id'] == $value['unit_id']))
        //     ) {
        //         dd("DDD");
        ...
        if ($save == 1) {
            return Redirect::to(url('/') .   '/admin/create-products-colors/' . $product->id);
        }
